...
class TokenBoxes {
  constructor(canvasId) {
    ...
    window.addEventListener('message', (event) => {
      if (event.data.type === 'PLEDGE_SUBMITTED') {
        this.queueTokenAnimation(event.data.boxNumber || 1, event.data.tokens || 1);
      }
    });
  ...
  animateTokenFall(boxNumber) {
    ...
    const targetY = boxRect.top - canvasRect.top + 20;
  ...
  animateToken(token) {
    ...
    if (token.progress < 1) {
      ...
      this.boxCounts[token.targetX > this.width / 2 ? 5 : 1]++;
      this.total++;
