...
class VegetablePatch {
  ...
  update(total, goal) {
    ...
    // Calculate growth rate based on difference
    const diff = this.percentage - this.displayPercentage;
    this.currentGrowthRate = Math.max(0.1, Math.abs(diff) * 0.1);
  ...
  animateGrowth() {
    ...
    if (Math.abs(diff) > 0.1) {
      // Still animating
      const step = diff > 0 ? this.currentGrowthRate : -this.currentGrowthRate;
  ...
  render() {
    ...
    // Calculate number of vegetables to show based on display percentage (for animation)
    ...
    for (let row = 0; row < 6; row++) {
      ...
      for (let col = 0; col < 6; col++) {
        ...
        // Calculate growth stage (0-1) - add randomness for natural look
        const growthProgress = Math.min(1, (vegCount / (vegetablesToShow || 1)) + 0.3 + Math.random() * 0.2);
    ...
    ctx.lineWidth = 4;
    const progressText = `${this.displayPercentage.toFixed(1)}% Grown`;
